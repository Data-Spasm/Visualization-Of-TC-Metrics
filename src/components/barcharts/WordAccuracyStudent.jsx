    ...
    if (student) {
      ...
      if (validMiscues.length > 0) {
        const studentData = validMiscues.map((entry, index) => ({
          ...
          passage: abbreviateTitle(entry.passageTitle || `Passage ${index + 1}`),
          correctWords: entry.numCorrect || 0,
        }));

        console.log("Processed from miscues prop:", studentData);
  ...
  return (
    <div className="chart-container">
      ...
      <ResponsiveContainer width="100%" height={420}>
        <BarChart data={processedData} margin={{ top: 20, right: 30, left: 20, bottom: 65 }}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis
            dataKey="passage"
            label={{ value: "Passages", position: "insideBottom", offset: -40 }}
            interval={0}
            angle={-30}
            textAnchor="end"
            height={80}
